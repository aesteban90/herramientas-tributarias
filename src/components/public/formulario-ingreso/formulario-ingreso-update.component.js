...
export default class FormularioIngreso extends Component{
        ...
        axios.get(configData.serverUrl + `/formularioIngreso/${this.props.match.params.id}`)
            .then(response => {
                ...
                this.setState({
                    ...
                    tipoIngresoSelected: {value:'', label: data.tipoIngreso},
    ...
    calcularRentaNeta = () =>{
        if(this.state.tipoIngresoSelected.label.indexOf('516')>-1){
            ...
            if(this.state.tipoIngresoSelected.label !== "Autovehículos (Cuando Costo Adquisición fue totalmente deducido)"){
                //Si es <> Autoveihiculos       
                rentaNetaReal = montoVenta - costoDeducido;
                rentaNetaReal = (rentaNetaReal < 0 ? 0 : rentaNetaReal);

                if(this.state.tipoIngresoSelected.label !== "Rescate total o parcial de acciones"
                    && this.state.tipoIngresoSelected.label !== "Rescate total o parcial de Cuotas partes"){
    ...
    onSubtmit = (e) => {
        e.preventDefault();
        axios.post(configData.serverUrl + '/formularioIngreso/update/'+this.props.match.params.id,this.state)
