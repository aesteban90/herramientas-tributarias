...
export default class FormularioEgreso extends Component{
    ...
    async componentDidMount(){   
        ...
        await axios.get(configData.serverUrl + "/users/informante/"+UserLogueado.id)
            .then(response => {
        ...
        await axios.get(configData.serverUrl + "/tipo-identificacion/egreso/")
        .then(response => {
            options = [];
        ...
        await axios.get(configData.serverUrl + "/tipo-comprobante/egresos-compras/")
        .then(response => {
            options = [];
            response.data.forEach(element => {
                options.push({value:element.codigo,label:element.descripcion});
        ...
        /*Tipo Egreso Opciones*/
        await axios.get(configData.serverUrl + "/tipo-egreso/")
        .then(response => {
            options = [];
            response.data.forEach(element => {
        ...
        //Obtiene los datos del formulario para actualizar
        this.getDatosFormulario();
    }

    getDatosFormulario = () => {
        axios.get(configData.serverUrl + `/formularioEgreso/${this.props.match.params.id}`)
            .then(response => {
                const data = response.data;
                console.log(data)
                this.setState({
                    tipoIdentificacionSelected: {value:data.identificacionCodigo,label:data.identificacion},
                    RucsSelected: {value:"",label:data.ruc+" | "+data.razonSocial},
                    fechaPago: new Date(data.fechaPago),
                    fechaEmision: new Date(data.fechaEmision),
                    tipoComprobanteSelected: {value:data.comprobanteTipoCodigo,label:data.comprobanteTipo},
                    comprobanteNumero: data.comprobanteNumero,
                    condicionSelected: {value:data.condicionCodigo,label:data.condicion},
                    excenta: data.excentas,
                    IVA5: data.iva5Incluido,
                    IVA10: data.iva10Incluido,
                    isImputaIVA: data.imputaIVA,
                    isMonedaExtranjera: data.monedaExtranjera,
                    tipoEgresoSelected: {value:'', label: data.tipoEgreso},
                    isGravadoIRP: data.gravadoIRP,
                    montoVenta: data.montoVenta,
                    totalIVA: data.totalIVA,
                    totalComprobante: data.totalComprobante,
                    costoDeducido: data.costoDeducido,
                    rentaNetaReal: data.rentaNetaReal,
                    rentaNetaPresunta: data.rentaNetaPresunta,
                    rentaNetaImponible: data.rentaNetaImponible,
                    timbradoSelected: {value:'',label:data.informanteTimbrado},
                    comprobanteAsociado: data.comprobanteAsociado,
                    timbradoAsociado: data.timbradoAsociado
                }, () => this.calcularMontoCompra());
                console.log(this.state)
                this.onChangeTipoEgresoOptions(this.state.tipoEgresoSelected);
    ...
    onChangeUtilizaIVA(isChecked){this.setState({isImputaIVA: isChecked})};
    ...
    onSubtmit = (e) => {
        e.preventDefault();
        axios.post(configData.serverUrl + '/formularioEgreso/update/'+this.props.match.params.id,this.state)
    ...
    render(){             
        return(
            <div className="container">
                ...
                <form id="formEgreso" name="formEgreso" onSubmit={this.onSubtmit}>
                    <div className="row col-md-12">
                        ...
                        <div className="row col-md-6 text-right">
                            ...
                            <div className="form-group col-md-3">
                                ...
                                <input type="submit" name="submit" id="submit" value="Submit" className="submit d-none" />
                    ...
                    <div className="row col-md-12">
                        ...
                        <div className="row col-md-12">
                            ...
                            <div className="form-group col-md-1 d-block" >
                                <label>Utiliza IVA: </label><br/>
                                    <Switch idToggle="check_utiliza_iva" isToggled={this.state.isImputaIVA} onToggle={() => this.onChangeUtilizaIVA(!this.state.isImputaIVA)} />                       
                            </div>      
