...
import TipoDocumentoEgreso from './tpdoc-egreso-form.component';
import Modal from '../../modals/modal';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faPlus, faEdit, faTrash } from '@fortawesome/free-solid-svg-icons'
const configData = require('../../../config.json');

export default class TipoDocumentoEgresoList extends Component{
    constructor(props){
        super(props);
        this.state = {
            datos: [],
    ...
    updateList = async (value) => {
        await axios.get(configData.serverUrl + "/tipo-documento-egreso/")
            .then(response => {
    ...
    deleteData = async (jsondatos) => {
        this.setState({
            ...
            modalTitle: 'Esta seguro de eliminar el Tipo Documento Egreso?',
            modalBody: 'Tipo: '+jsondatos.tipo+' \nCódigo: '+jsondatos.codigo+'\nDescripción: '+jsondatos.descripcion 
        })
    }    
    actionConfirmModal = () =>{
        if(this.state.modalAction === "delete"){
            axios.delete(configData.serverUrl + "/tipo-documento-egreso/" + this.state.modalID)
        ...
        return(
            <div className="container">
                ...
                <div className="row mb-0 col-md-12">
                    <h3>Lista de Tipos de Documentos de Egresos</h3>
                </div>
                <div className="row">
                    ...
                    <div className="col-md-4">
                        <TipoDocumentoEgreso idUpdate={this.state.idUpdate} onUpdateParentList={this.updateList}/>
                    </div>
                </div>
            </div>
        )
