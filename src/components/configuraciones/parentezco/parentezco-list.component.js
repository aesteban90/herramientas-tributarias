...
import Parentezco from './parentezco-form.component';
import Modal from '../../modals/modal';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faPlus, faEdit, faTrash } from '@fortawesome/free-solid-svg-icons'
const configData = require('../../../config.json');

export default class ParentezcoList extends Component{
    constructor(props){
        super(props);
        this.state = {
            datos: [],
    ...
    updateList = async (value) => {
        await axios.get(configData.serverUrl + "/parentezco/")
            .then(response => {
    ...
    deleteData = async (jsondatos) => {
        this.setState({
            ...
            modalTitle: 'Esta seguro de eliminar el Parentezco?',
            modalBody: 'Código: '+jsondatos.codigo+'\nDescripción: '+jsondatos.descripcion 
        })
    }    
    actionConfirmModal = () =>{
        if(this.state.modalAction === "delete"){
            axios.delete(configData.serverUrl + "/parentezco/" + this.state.modalID)
            ...
            <div className="container">
                ...
                <div className="row">
                    ...
                    <div className="col-md-4">
                        <Parentezco idUpdate={this.state.idUpdate} onUpdateParentList={this.updateList}/>
                    </div>
                </div>
            </div>
        )
